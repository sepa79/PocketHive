...
ALL_SERVICES=(rabbitmq log-aggregator scenario-manager orchestrator tcp-mock-server ui ui-v2 prometheus grafana loki wiremock pushgateway redis redis-commander swarm-controller generator http-builder request-builder moderator processor postprocessor clearing-export trigger)
declare -A DURATIONS=()
...
clean_stack() {
  ...
  if $PRUNE_IMAGES; then
    ...
    mapfile -t ph_images < <(docker images --format '{{.Repository}} {{.ID}}' | awk '
      $1 ~ /^(orchestrator|scenario-manager|log-aggregator|tcp-mock-server|ui|swarm-controller|generator|moderator|processor|postprocessor|clearing-export|trigger|pockethive-)/ { print $2 }')
