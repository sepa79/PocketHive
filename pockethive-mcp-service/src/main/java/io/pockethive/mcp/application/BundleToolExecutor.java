...
/**
 * Responsibility: Execute MCP-owned bundle validation and publication-ticket tools. Must not:
 * Capture QA requirements, manage sessions, or publish directly to Scenario Manager. Contract:
 * RESP-MCP-UPLOAD-COORDINATION -
 * docs/architecture/runtime-responsibilities.md#resp-mcp-upload-coordination.
 */
@Component
final class BundleToolExecutor {
  ...
  BundleToolExecutor(
      BundleUploadCoordinator uploads,
    ...
    this.actions =
        Map.of(
            ...
            McpToolId.SCENARIO_BUNDLE_DIRECT_VALIDATION_PREPARE,
                (input, caller) ->
                    validationTicketView(
                        uploads.prepareDirectValidationWithCapability(
                            caller.principal(), source(input), manifest(input), clock.instant())),
            McpToolId.SCENARIO_BUNDLE_VALIDATION_RECEIPT_GET,
                (input, caller) ->
                    BundleValidationReceiptView.from(
            ...
            McpToolId.SCENARIO_BUNDLE_PUBLICATION_ATTEMPT_GET,
                (input, caller) ->
                    PublicationAttemptView.from(
                        uploads.publicationAttempt(text(input, "attemptId"), caller.principal())),
            McpToolId.SCENARIO_BUNDLE_PUBLICATION_RECONCILE,
                (input, caller) ->
                    PublicationAttemptView.from(
  ...
  private Object prepareValidation(Map<String, Object> input, McpCaller caller) {
    ...
    return validationTicketView(
        uploads.prepareValidationWithCapability(
            caller.principal(),
            UploadWorkflowBinding.workflow(workflow),
            source(input),
            manifest(input),
            clock.instant()));
  }
  ...
  private Object preparePublication(Map<String, Object> input, McpCaller caller) {
    ...
    try {
      ...
      return publicationTicketView(
          uploads.preparePublicationWithCapability(
              caller.principal(),
              receiptId,
              PublicationMode.valueOf(text(input, "mode").toUpperCase(Locale.ROOT)),
              scenarioId,
              source(input),
              manifest(input),
              text(input, "archiveDigest"),
              text(input, "bundleContentDigest"),
              clock.instant()));
  ...
  private ValidationUploadTicketView validationTicketView(
    ...
    return new ValidationUploadTicketView(
        ticket.id(), uploadUrl(ticket.id()), prepared.uploadCapability(), ticket.expiresAt());
  }
  ...
  private PublicationUploadTicketView publicationTicketView(
    ...
    return new PublicationUploadTicketView(
        ticket.id(),
        uploadUrl(ticket.id()),
        prepared.uploadCapability(),
        ticket.expiresAt(),
        ticket.attemptId(),
        ticket.validationReceiptId(),
        ticket.mode(),
        ticket.scenarioId());
  }
          ...
          list.stream()
              .map(
                  item -> {
                    ...
                    return new BundleFileManifestEntry(
                        text(entry, "path"), number(entry, "byteCount"), text(entry, "sha256"));
                  })
              .toList());
    } catch (IllegalArgumentException exception) {
