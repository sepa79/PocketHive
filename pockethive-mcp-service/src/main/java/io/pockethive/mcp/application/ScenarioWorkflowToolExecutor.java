...
final class ScenarioWorkflowToolExecutor {
  ...
  ScenarioWorkflowToolExecutor(
      WorkflowAccess workflows,
      OwnerApiPort owners,
      PocketHiveMcpProperties properties,
      ObjectMapper mapper,
      CoordinationStateRepository state,
  ...
  Object execute(
      McpToolId toolId, ClientInteraction exchange, McpCaller caller, Map<String, Object> input) {
  ...
  private Object create(Map<String, Object> input, McpCaller caller) {
    ...
    session.addWorkflow(
        number(input, "expectedSessionRevision"), id, properties.maxWorkflowsPerSession());
  ...
  private Object answer(ClientInteraction exchange, McpCaller caller, Map<String, Object> input) {
    ...
    QaRequirementQuestionContract question =
        QaRequirementQuestionContract.forTopic(topic, QaAnswerCaptureMode.MCP_FORM);
    ClientElicitationResult result = exchange.elicit(question.message(), question.responseSchema());
    if (result.action() != io.pockethive.mcp.domain.ElicitationAction.ACCEPT) {
      return Map.of(
          "workflowId", workflow.id(),
          "state", workflow.state(),
          "revision", workflow.revision(),
          "topic", topic,
          "elicitationAction", result.action().name().toLowerCase(Locale.ROOT),
          "disposition", "UNKNOWN");
    }
    return applyAcceptedAnswer(
        exchange, caller, workflow, expectedRevision, question, result.content());
  }

  private Object question(Map<String, Object> input, McpCaller caller) {
    ScenarioWorkflow workflow = workflows.requireMutableWorkflow(text(input, "workflowId"), caller);
    QaRequirementQuestionContract question =
        QaRequirementQuestionContract.forTopic(topic(input), QaAnswerCaptureMode.AGENT_MEDIATED);
  ...
  private Object submitAnswer(
      ClientInteraction exchange, McpCaller caller, Map<String, Object> input) {
    ScenarioWorkflow workflow = workflows.requireMutableWorkflow(text(input, "workflowId"), caller);
    long expectedRevision = number(input, "expectedRevision");
    ...
    for (QaRequirementReviewContract.ReviewAnswer answer : review.answers()) {
      AnswerProvenance provenance =
          new AnswerProvenance(
              ...
              workflow.id(),
              expectedRevision,
              QaAnswerCaptureMode.COMPACT_REVIEW.questionId(answer.topic()),
  ...
  private static RequirementAnswer reviewRequirement(
      QaRequirementReviewContract.ReviewAnswer answer,
    ...
    if (answer.disposition()
        == io.pockethive.mcp.domain.RequirementDisposition.USER_CONFIRMED_SOURCE) {
  ...
  private Map<String, Object> reviewView(
      ScenarioWorkflow workflow, QaRequirementReviewContract review) {
  ...
  private QaRequirementReviewContract reviewContract(Map<String, Object> input) {
    return QaRequirementReviewContract.parse(
        require(input, "answers"),
        optionalText(input, "sourceName"),
        optionalText(input, "sourceDigest"));
  }
  ...
  private Object applyAcceptedAnswer(
    ...
    AnswerProvenance provenance =
        new AnswerProvenance(
            ...
            expectedRevision,
            question.questionId(),
            schemaDigest(question),
            ElicitationAction.ACCEPT,
            contentDigest,
            clock.instant());
    RequirementAnswer requirement =
        switch (disposition) {
          case "USER_PROVIDED" -> {
            rejectSourceFields(content);
            yield RequirementAnswer.userProvided(answer, provenance);
          }
          case "USER_CONFIRMED_SOURCE" ->
              RequirementAnswer.userConfirmedSource(
                  answer,
                  new ConfirmedSource(text(content, "sourceName"), text(content, "sourceDigest")),
                  provenance);
          case "NOT_APPLICABLE" -> {
            rejectSourceFields(content);
          ...
          default ->
              throw new ToolExecutionException("REQUIREMENT_DISPOSITION_INVALID", disposition);
        };
  ...
  private Object generate(Map<String, Object> input, McpCaller caller) {
    ...
    workflow.readyToGenerate(
        expectedRevision, new CapabilityFingerprint(sha256(json(capability)), clock.instant()));
  ...
  private List<Map<String, Object>> fileList(Object value) {
    ...
    return files.stream()
        .sorted(
            (left, right) ->
                String.valueOf(left.get("path")).compareTo(String.valueOf(right.get("path"))))
        .toList();
  }

  private static void requireFormElicitation(ClientInteraction exchange) {
    if (!exchange.supportsForm()) {
      throw new ToolExecutionException(
          "ELICITATION_CAPABILITY_REQUIRED", "The client must support MCP form elicitation");
    }
  }
  ...
  private static void rejectSourceFields(Map<String, Object> input) {
    if (input.containsKey("sourceName") || input.containsKey("sourceDigest")) {
      throw new ToolExecutionException(
          "REQUIREMENT_SOURCE_FORBIDDEN", "sourceName/sourceDigest require USER_CONFIRMED_SOURCE");
    }
  }
  ...
  private interface ToolAction {
    Object execute(ClientInteraction exchange, McpCaller caller, Map<String, Object> input);
  }
}
