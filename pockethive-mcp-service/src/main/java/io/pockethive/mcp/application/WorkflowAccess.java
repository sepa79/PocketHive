...
/**
 * Responsibility: Enforce principal and session-state access to MCP-owned authoring workflows. Must
 * not: Mutate workflow content, build projections, or execute authoring actions. Contract:
 * docs/mcp/README.md.
 */
@Component
final class WorkflowAccess {
  ...
  WorkflowAccess(
      CoordinationStateRepository state, PocketHiveMcpProperties properties, Clock clock) {
    this.state = state;
