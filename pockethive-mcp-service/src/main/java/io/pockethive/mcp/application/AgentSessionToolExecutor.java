...
final class AgentSessionToolExecutor {
  ...
  AgentSessionToolExecutor(
    ...
    this.actions =
        Map.of(
            ...
            McpToolId.AGENT_SESSION_GET,
                (caller, input) ->
                    projection.session(
                        workflows.requireSession(text(input, "agentSessionId"), caller)),
            McpToolId.AGENT_SESSION_LIST_WORKFLOWS,
                (caller, input) ->
                    listWorkflows(workflows.requireSession(text(input, "agentSessionId"), caller)),
            McpToolId.SCENARIO_WORKFLOW_LIST,
                (caller, input) ->
                    listWorkflows(workflows.requireSession(text(input, "agentSessionId"), caller)),
  ...
  private Object create(McpCaller caller) {
    ...
    AgentSession session =
        AgentSession.open(id, caller.principal(), now, properties.openSessionTtl());
  ...
  private Map<String, Object> listWorkflows(AgentSession session) {
    List<Map<String, Object>> items =
        state.findWorkflows(session.workflowIds()).stream().map(projection::workflow).toList();
