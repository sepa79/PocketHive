...
/**
 * Responsibility: decode authenticated MCP transport identity once. Must not: grant scopes or own
 * workflow transitions. Contract: RESP-MCP-CLIENT-INTERACTION —
 * docs/architecture/runtime-responsibilities.md#resp-mcp-client-interaction.
 */
public final class McpCallerDecoder {
  ...
  public static io.pockethive.mcp.application.McpCaller from(McpTransportContext context) {
    ...
    Set<String> scopes =
        Arrays.stream(require(context, SCOPES).split(" "))
            .filter(value -> !value.isBlank())
            .collect(Collectors.toUnmodifiableSet());
    return new io.pockethive.mcp.application.McpCaller(
        new PrincipalKey(URI.create(issuer), subject), principalLabel, clientId, scopes);
  }
