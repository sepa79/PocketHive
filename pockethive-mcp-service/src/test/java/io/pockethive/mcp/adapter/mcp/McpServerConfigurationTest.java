...
class McpServerConfigurationTest {
  ...
  void publishesCanonicalToolsAndPortableKnowledgeOnTheExactProtocolEndpoint() {
    ...
    EnvironmentHealthService environmentHealth =
        new EnvironmentHealthService(
            properties.pocketHiveIngress(),
            List.of(
                new EnvironmentHealthTarget(
                    "ui", "UI", URI.create("/"), "/healthz", EnvironmentHealthContract.PLAIN_OK)),
            target -> true,
            Clock.systemUTC());
    HttpServletStreamableServerTransportProvider transport =
        configuration.transport(objectMapper, properties);
    McpKnowledgeResources resources =
        new McpKnowledgeResources(
            new io.pockethive.mcp.application.McpKnowledgeProjection(
                catalogue,
                properties,
                objectMapper,
                Clock.systemUTC(),
                new ClasspathKnowledgeDocuments(objectMapper)),
            environmentHealth,
            objectMapper,
            transport);
    ...
    McpSyncServer server =
        configuration.server(
            ...
            new ToolFailureMapper(),
            objectMapper,
            new BuildProperties(buildValues));
    ...
    assertThat(server.listResources())
