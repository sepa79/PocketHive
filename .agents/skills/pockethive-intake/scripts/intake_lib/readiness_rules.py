...
# Intake representations with detailed decision checks; the runtime type catalogue
# belongs to the generated schema and is deliberately broader than these cases.
_AUTH_NONE = "none"
_AUTH_OAUTH_CLIENT_CREDENTIALS = "oauth2-client-credentials"
_AUTH_BEARER_TOKEN = "bearer-token"
...
def check_readiness(docs: dict) -> tuple[list[dict], list[dict]]:
    ...
    for index, execution in enumerate(plan["apiExecution"]):
        ...
        if auth_type == _AUTH_OAUTH_CLIENT_CREDENTIALS:
        ...
        elif auth_type == _AUTH_BEARER_TOKEN and api["authorization"].get("tokenSource") is not None:
        ...
        elif auth_type not in (None, _AUTH_NONE):
            issue(gaps, "AUTH_REPRESENTATION", "requirements", api_base + "/authorization/type",
                  "The auth type is recognized, but its acquisition, application and credential decisions need a reviewed intake representation before handoff.")
    ...
    if protected:
        ...
        for index, binding in enumerate(plan["secretInjection"]["bindings"]):
            ...
            if kind in ("file", "vault"):
                need("plan", base + "/resolverRef")
            if kind in ("env", "file", "vault"):
