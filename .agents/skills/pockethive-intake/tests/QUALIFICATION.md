...
## Runtime vocabulary and object ownership — 1.9.0, 2026-09-22

The audit covered shared object definitions, all four document schemas and
bundle observations against the repository's canonical contracts.

| Area | Finding and treatment |
| --- | --- |
| Auth types | The old three-value intake list omitted canonical types. A generated projection of compiled `AuthType.values()/key()` now supplies all 13 keys. Unknown draft choice remains null; unknown names fail validation. Detailed unsupported representations remain handoff gaps. |
| Request protocols | Observation vocabulary now derives from `RequestTemplateProtocol`, including ISO8583. Protocol and endpoint kind remain separate contracts. HTTP requests can reference HTTPS endpoints. |
| SUT/endpoints | Canonical `SutEndpoint.upstreamBaseUrl` was missing and is now retained. SUT classifications and endpoint kinds remain open descriptions. Intake access/OAuth requirements are intentionally distinct from runtime SUT objects. |
| Secret injection | Plan/results share one intake type definition. File injection is included and requires its explicit resolver and field mapping. These records never resolve secrets. |
| Plan/results duplication | Six repeated vocabularies now share definitions: data-source type, API execution mode, workload relationship, rate unit, arrival model and dependency disposition. Observed API runtime fields reference the planned fields' constraints. |
| Run identity | Results previously rejected canonical `runId`. It is now nullable in the scaffold and required alongside reusable `swarmId` for executed reports. An allocated ID does not establish execution. |
| Data and bindings | Database engines and column types remain open. Dataset/binding choices describe intake requirements, not worker adapter settings. Observed source-type text remains open to preserve incomplete or unexpected evidence. |
| Representation limits | Generator rules, response checks and TCP/ISO request details are limited intake models, not copies of runtime functions, `ResultRules` or request-template DTOs. Keep unrepresented requirements in source evidence and explicit review questions; schema acceptance is not proof of full runtime representation. |

Requirements version 3 replaces the intake alias `bearer` with canonical
`bearer-token`; migration is explicit. Plan, traceability and results versions
remain 5, 2 and 2. Package contract version is 8. Original source snapshots and
their checksum list remain unchanged.

The compiled-owner Java contract tests passed **2 tests**. The generator's
read-only check passed, and missing/stale projections were rejected without
mutation. Expanded document schemas before/after the broader consolidation were
equivalent, ignoring enum order and the deliberate addition of `runInfo.runId`.
No runtime scenario validation, live authentication or load execution was run.

The final expanded candidate passed **265 public-CLI tests** with zero failures,
errors or skips in **191.291 seconds** total wall time across four shards.
Python 3.12.3 and Bubblewrap 0.9.0 ran the extracted package read-only, with host
home/repository absent, external networking unavailable and site packages
disabled. The isolation probe passed and package bytes remained unchanged.
Tests include all canonical auth choices, explicit old-version rejection,
handoff representation gaps, protocol redaction and evidence-linked run identity.

Tested candidate ZIP SHA-256:
`3b86ad306c7c37f27afbfea66059ab6e8a8bb40d86e3beeb4d7b1afd29daca66`.
The final package adds this report, restores pre-existing schema formatting
without changing its parsed content, and refreshes manifest hashes. Test and
runtime code match the qualified candidate. These are local development checks;
native client behaviour and deployed PocketHive capabilities remain unqualified.

Local evidence, not package dependencies:
`/home/tim/.tmp/pip/pockethive-intake-1.9.0-expanded-offline-ap4q74c9/`,
`/home/tim/.tmp/pip/pockethive-intake-vocabulary-17anrg1g/`, and
`/home/tim/.tmp/pip/intake-schema-equivalence-e5noczsp/`.
