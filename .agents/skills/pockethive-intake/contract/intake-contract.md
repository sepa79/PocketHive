# Intake document and CLI contract — version 8
...
`runInfo.runId` retains the canonical runtime run identity separately from
`swarmId`, which can be reused across runs. It starts unknown and is required
alongside the existing identity fields for an executed report. Never infer it
from a swarm name, timestamp or result-document ID. Recording an evidenced run
identity does not itself establish execution or a passing outcome.
...
### Runtime vocabulary ownership

`schemas/runtime-vocabulary.schema.json` is a generated, read-only projection of
the compiled `AuthType` and `RequestTemplateProtocol` contract types. The
requirements authorization schema references its complete canonical auth keys;
the inspector uses its protocol list for safe observations. Generation and the
repository drift check run during maintenance, never during an intake. Installed
packages use only their sealed local files. Do not parse Java source, maintain a
second type list, fetch schemas, or infer support from worker-role names.

Intake-owned concepts such as approval states, data preparation and workload
classifications remain separate from runtime types. The observation allowlist
is a redaction boundary, not a complete scenario validator. Custom method and
endpoint-kind values may remain unextracted and explicitly require source review.
API protocol and endpoint kind are different contracts: reference validation
...
`tokenSource` under that type. There is no implicit alias or document migration.
All canonical auth types are valid draft values; unknown names fail schema
validation. Types without a complete intake representation report
`AUTH_REPRESENTATION` at handoff rather than becoming ready from their name alone.
Current detailed representations are no auth, client-credentials OAuth and an
explicit upstream bearer-token provider. Static bearer use without that provider
also remains a representation gap. OAuth client-authentication choices describe
requirements and do not certify worker support for the selected mechanism.

Secret injection has one shared intake type definition for plan and results:
`env`, `file`, or `vault`, with `null` for an unknown draft choice. File injection
records mounted-file references; Vault denotes an explicit external arrangement,
not a claimed native runtime resolver. Both require an explicit `resolverRef` and
nonempty `fieldMapping` for each binding. These declarations never resolve secrets.

Shared intake vocabularies have one definition in `common.schema.json`:
data-source type, API execution mode, workload relationship, rate unit, arrival
model and dependency disposition. Plans and results reference those definitions;
they do not maintain independent lists. Observed data-source descriptions remain
text so incomplete drafts can preserve unexpected observations; reference checks
still compare them with the selected requirements source when both are known.
Observed API runtime fields reuse the planned fields' schema constraints.

Intake objects are not copies of worker configuration. Database engines, column
types, SUT classifications and pipeline roles remain open descriptions. Generator
rules and response checks are limited intake representations, not catalogues of
runtime template functions or result-rule operators. Requirements needing other
parameters or predicates must retain their source evidence and an explicit
representation-review question; a recognized name does not establish complete
capture. TCP/ISO transport details continue to report their representation gap.
