...
class PublicEndpointTest(unittest.TestCase):
    def render(self, action, profile, ingress, allowance):
        runtime = {
            ...
            "POCKETHIVE_AUTH_OAUTH_INTROSPECTION_SECRET": "fixture-secret-not-real",
            "POCKETHIVE_AUTH_SERVICE_ACCOUNT_MCP_SECRET": "fixture-secret-not-real",
        }
        ...
        public_guard = next(task["ansible.builtin.assert"] for task in tasks
                            if task["name"] == "Require explicit PocketHive MCP public identity and secrets")
