...
public class LogAggregator {
  ...
  public LogAggregator(
      @Value("${pockethive.loki.url:${ph.loki.url:${PH_LOKI_URL:http://loki:3100}}}") String lokiBase,
      @Value("${pockethive.loki.max-retries:${ph.loki.maxRetries:3}}") int maxRetries,
      @Value("${pockethive.loki.backoff-ms:${ph.loki.backoffMs:500}}") long backoffMs,
      @Value("${pockethive.loki.batch-size:${ph.loki.batchSize:100}}") int batchSize) {
  ...
  @RabbitListener(queues = "${pockethive.logs.queue:${ph.logsQueue:ph.logs.agg}}")
  ...
  @Scheduled(fixedRateString = "${pockethive.loki.flush-interval-ms:${ph.loki.flushIntervalMs:1000}}")
  public void flush(){
