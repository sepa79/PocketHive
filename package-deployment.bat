...
set "PACKAGE_STAGING=%CD%\.%PACKAGE_NAME%.%RANDOM%-%RANDOM%.tmp.zip"
set "PACKAGE_BACKUP=%PACKAGE_STAGING%.backup"
set "PACKAGE_STAGING_OWNED="
set "PACKAGE_BACKUP_OWNED="
if exist "%PACKAGE_STAGING%" goto package_failed
if exist "%PACKAGE_BACKUP%" goto package_failed
...
set "PACKAGE_STAGING_OWNED=1"
powershell.exe -NoProfile -NonInteractive -Command "$ErrorActionPreference='Stop'; Compress-Archive -LiteralPath $env:DEPLOY_DIR -DestinationPath $env:PACKAGE_STAGING"
if errorlevel 1 goto package_failed
if not exist "%PACKAGE_STAGING%" goto package_failed
for %%A in ("%PACKAGE_STAGING%") do if %%~zA LEQ 0 goto package_failed

rem Publish only after the new archive is complete. Staging is on the same
rem volume as the final archive so replacement cannot delete an older valid
rem package because an earlier copy or compression step failed.
set "PACKAGE_BACKUP_OWNED=1"
powershell.exe -NoProfile -NonInteractive -Command "$ErrorActionPreference='Stop'; if ([System.IO.File]::Exists($env:PACKAGE_OUTPUT)) { [System.IO.File]::Replace($env:PACKAGE_STAGING, $env:PACKAGE_OUTPUT, $env:PACKAGE_BACKUP, $true); [System.IO.File]::Delete($env:PACKAGE_BACKUP) } else { [System.IO.File]::Move($env:PACKAGE_STAGING, $env:PACKAGE_OUTPUT) }"
if errorlevel 1 goto package_failed
set "PACKAGE_STAGING_OWNED="
set "PACKAGE_BACKUP_OWNED="
...
if defined PACKAGE_STAGING_OWNED if exist "%PACKAGE_STAGING%" del /Q "%PACKAGE_STAGING%" >nul 2>&1
if defined PACKAGE_BACKUP_OWNED if exist "%PACKAGE_BACKUP%" del /Q "%PACKAGE_BACKUP%" >nul 2>&1
