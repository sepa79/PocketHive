...
const ARCHITECTURE_DETAILS: Record<ArchitectureDetailKey, ArchitectureDetail> = {
  orchestrator: {
    title: "Orchestrator",
    description:
      "Owns runtime and workload intent, asynchronous operations, startup artifacts, lifecycle commands, and every public terminal outcome.",
  },
  controller: {
    title: "Swarm Controller",
    description:
      "Loads and verifies its startup artifact, provisions workers, broadcasts enablement, aggregates status, and reports internal executor results after convergence.",
  },
  "control-queues": {
    title: "RabbitMQ control queues",
    description:
      "Carry command signals, executor results, Orchestrator-owned public outcomes, status metrics, journal events, and alerts. Business WorkItems do not pass through the control plane.",
  },
  "source-worker": {
    title: "Source worker",
    description:
  ...
  evidence: {
    ...
    description:
      "Use the operation, Journal, fresh lifecycle axes, logs, metrics, and Grafana to confirm what ran and whether the swarm converged.",
  },
  "multi-swarm": {
...
const LIFECYCLE_STEPS = [
  "Create accepted",
  "READY + STOPPED observed",
  "CREATE SUCCEEDED",
  "START SUCCEEDED + RUNNING",
  "STOP SUCCEEDED + STOPPED",
  "REMOVE SUCCEEDED + absent",
];
    ...
    >
      <div className="iph-card-lead iph-card-lead--split">
        <div>
          ...
          <strong className="iph-route-title">Follow operation and observation separately</strong>
        </div>
        <p>Terminal operation evidence and fresh current axes answer different questions.</p>
      </div>
      ...
      <div className="iph-proof-grid">
        ...
        <div>
          <strong>2 &middot; Executor result</strong>
          <span>The target reports internal correlated evidence when applicable.</span>
        </div>
        <div>
          <strong>3 &middot; Terminal outcome</strong>
          <span>The Orchestrator publishes the public result after postconditions.</span>
        </div>
        <div>
          <strong>4 &middot; Fresh axes</strong>
          <span>Current intent, observation, health, and resources remain explicit.</span>
        </div>
      </div>
    </section>
  );
