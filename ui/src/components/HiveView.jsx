...
import { initHiveMenu } from '../features/hive.js';

export default function HiveView() {
  useEffect(() => {
    const hive = initHiveMenu();
    hive.ensureNode('generator', 'Generator');
    hive.ensureNode('moderator', 'Moderator');
    hive.ensureNode('processor', 'Processor');
    hive.redrawHive();
  }, []);
  return (
    <div id="view-hive">
      <div id="hive-toolbar">
        <label>Hold time (s) <input id="hive-hold" type="number" min="1" max="120" step="1" defaultValue="15" style={{ width: '64px' }} /></label>
