...
public class AuthSessionService {
    ...
    public AuthSessionService(AuthServiceProperties properties, InMemoryUserStore users) {
    ...
    public SessionResponseDto createSession(StoredUser user) {
        ...
        sessions.put(token, new AuthSession(token, user.id(), expiresAt));
    ...
    public AuthenticatedUserDto resolve(String token) {
        ...
        StoredUser user = users.requireById(session.userId());
        if (!user.active()) {
