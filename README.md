...
- Metrics/status flow back on the `ph.control` exchange, with each service listening on its own `ph.control.<role>.<instance>` queue for broadcasts and direct signals.
