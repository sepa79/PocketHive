...
const SCENARIO_PROTOCOL_VERSION = "2.0.0";
...
async function scenarioManagerDryRunValidateBundle(bundle) {
  const zipBytes = await createBundleZipBytes(bundle);
  try {
    const result = await httpJson(`${SM_URL}/validation/scenario-bundles`, {
      method: "POST",
      body: zipBytes,
      headers: { "content-type": "application/zip" },
      timeoutMs: 60000,
    });
    const evidence = result?.validation;
    for (const field of ["supportedScenarioProtocolVersion", "scenarioManagerVersion", "artifactDigest"]) {
      if (typeof evidence?.[field] !== "string" || !evidence[field]) {
        throw new Error(`Scenario Manager validation response is missing validation.${field}`);
      }
    }
    return result;
  } catch (e) {
