...
> Archived: the RabbitMQ plan handoff described here was removed by the filesystem startup artifact design. This document is retained only as historical context for that replacement.

**Area:** Orchestrator runtime (`SwarmSignalListener`, `ScenarioTimelineRegistry`)  
**Status:** Superseded by filesystem startup artifact
