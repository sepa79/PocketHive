...
> Superseded because the planned `TopologyView.tsx` structure no longer exists. The current single-swarm graph is implemented in `ui-v2/src/pages/SwarmViewPage.tsx`; remaining enhancements are tracked in `docs/todo/hive-graph-followups.md`.
...
- No new features (multi‑SUT on canvas, scenario editing); those are covered by
  `docs/ui-v2/SCENARIO_WORKSPACE_PLAN.md` and related docs.
