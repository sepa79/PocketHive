...
| Surface | Status at tested lifecycle rewrite source (`0524165e`) | Use it for | Boundary |
| --- | --- | --- | --- |
| PocketHive MCP | **Current, repository-local; stdio customer path** | Guarded scenario authoring, validation, lifecycle, evidence, and governed cleanup for agents or IDE users | Includes file, remote, diagnostic, and destructive tools; read each mutation label. The HTTP implementation is not customer-qualified. See [MCP current versus target](pockethive-mcp-and-bundles.md#current-versus-target). |
