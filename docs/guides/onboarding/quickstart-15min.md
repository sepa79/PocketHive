...
description: Build PocketHive from a known source tag, run a manually time-boxed local evaluation, verify one swarm, and clean up safely.
---
...
| Starting point | A complete PocketHive `v0.15.35` source checkout |
| Time | About 15 minutes on a prepared machine with warm Docker and Maven caches; a cold first build can take longer |
| Expected outcome | Build PocketHive, run `docs-demo` for no more than 60 seconds, verify and remove it, then shut down the stack. Local data and build artifacts remain. |
| Last-tested PocketHive version | PocketHive `v0.15.35` |
...
- In `v0.15.35`, build cleanup force-removes local Docker containers whose
...
cd PocketHive
git checkout v0.15.35
```

For a new or existing checkout, verify the exact tag and working tree:
...
The first command must print exactly `v0.15.35`. The second command must print
nothing. Any other result means that you are not reproducing the last-tested
source state used by this walkthrough. Here, **last-tested** is a project
compatibility statement, not cryptographic source-provenance verification.
...
published for `v0.15.35`. A first build compiles several modules and downloads
...
[![Create a swarm from the local REST topology bundle](pathname:///img/guides/operators/create-swarm.png)](pathname:///img/guides/operators/create-swarm.png)

[Open the creation screenshot at full size](pathname:///img/guides/operators/create-swarm.png).
...
`READY` alone is not enough. There is no fixed wait time for `v0.15.35`. If
...
1. Select **Start** and wait until **Swarm status** is `RUNNING`.
2. Confirm **Snapshot** remains fresh and **Swarm health** is exactly
   `RUNNING`.
3. In **Scenario**, confirm `genA`, `modA`, `procA`, and `postA` are all
   `enabled` and `live`.
4. Immediately select **Stop**. Do not let the swarm run for more than 60
   seconds after it first reaches `RUNNING`.

[![Running docs-demo swarm in Hive](pathname:///img/guides/operators/hive-runtime.png)](pathname:///img/guides/operators/hive-runtime.png)

[Open the running-swarm screenshot at full size](pathname:///img/guides/operators/hive-runtime.png).
...
1. Wait for **Swarm status** to reach `STOPPED`.
...
:::tip You successfully ran PocketHive

The local swarm reached `RUNNING`, all four planned workers were live and
enabled, and you stopped the manually time-boxed workload against local
WireMock. This proves the documented PocketHive control and worker flow for
this source checkout. It does not prove an external SUT or a business result.
...
2. In **Journal**, require the `swarm-remove` outcome for `docs-demo` with
   status `Removed`.
3. Confirm that `docs-demo` disappears from **Hive**.

Do not reuse the swarm ID or shut down the Compose stack while removal is
uncertain. If the `Removed` outcome is missing, retain the swarm ID, failed
action, and timestamps and follow the lifecycle recovery guide.

## 7. Shut down PocketHive

Only after the `Removed` outcome is confirmed, run from the repository root:
...
<details>
<summary>The swarm does not become ready or Start returns NotReady</summary>
...
and do not run `docker compose down` until the `Removed` outcome is confirmed.
