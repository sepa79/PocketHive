...
| Explicit worker I/O | `FULL` | `BOUNDED` | One explicitly selected finite Scheduler role per Provider Run; logical topology and fully rendered runtime queues must agree through the terminal `MANAGED_DATASET CREATE_RECORD` binding. | Topology/queue agreement, competing-source and M0 capability/config tests. |
| Atomic creation | `FULL` | `BOUNDED` | Stable Provider Item IDs, per-binding feasibility against the frozen Provider Run, frozen topology/wiring digests, a drained completion barrier and exact-target seal transaction. Generic mutation is absent. | Source/topology/wiring, changed-retry, unattainable-plan, redelivery, in-flight race, duplicate completion, underfill and overfill tests. |
...
| Provider-created supply | `FULL` for initial fill | `BOUNDED` | One explicit finite Scheduler role plus agreeing topology, runtime wiring and configuration digests are frozen per Provider Run; it closes issuance, drains issued items and creates one sealed revision. | Source ambiguity, topology/wiring disagreement, changed retry, fill, completion-race, crash and failure tests. |
