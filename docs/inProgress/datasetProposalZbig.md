...
| Explicit worker I/O | `FULL` | `BOUNDED` | One explicit finite Scheduler authoring role per Provider Run, terminal `MANAGED_DATASET CREATE_RECORD` and explicit consumer input. | Exact topology/reachability and M0 capability/config tests. |
| Atomic creation | `FULL` | `BOUNDED` | Stable Provider Item IDs, per-binding feasibility against the frozen Provider Run, a drained completion barrier and exact-target seal transaction. Generic mutation is absent. | Source/topology, unattainable-plan, redelivery, in-flight race, duplicate completion, underfill and overfill tests. |
...
| Provider-created supply | `FULL` for initial fill | `BOUNDED` | One exact finite Scheduler role/configuration is frozen per Provider Run; it closes issuance, drains issued items and creates one sealed revision. | Source ambiguity, fill, completion-race, crash and failure tests. |
