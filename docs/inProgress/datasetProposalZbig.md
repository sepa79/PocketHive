...
| Explicit worker I/O | `FULL` | `BOUNDED` | One explicitly selected finite Scheduler role per Provider Run; bounded relevant-subgraph comparison makes logical topology and every possible resolved runtime connection agree through the terminal `MANAGED_DATASET CREATE_RECORD` binding. | Linear/branch/cycle, duplicate-suffix, topology/runtime disagreement, competing-source and M0 capability/config tests. |
| Atomic creation | `FULL` | `BOUNDED` | Stable Provider Item IDs, per-binding feasibility, closed canonical topology/wiring digests, a drained completion barrier and exact-target seal transaction. Generic mutation is absent. | Canonical ordering/change vectors, changed-retry, unattainable-plan, redelivery, in-flight race, duplicate completion, underfill and overfill tests. |
...
| Provider-created supply | `FULL` for initial fill | `BOUNDED` | One explicit finite Scheduler role plus canonical topology, runtime wiring and configuration digests are frozen per Provider Run; it closes issuance, drains issued items and creates one sealed revision. | Source ambiguity, bounded graph agreement, canonical digest, changed retry, fill, completion-race, crash and failure tests. |
...
| Bounded status without value exposure | `FULL` | `BOUNDED` | REST/MCP separate reporter freshness, load and guarded-attempt freshness; observation windows cover qualified status delivery and reports are monotonic. | Delay, duplicate, reorder, clock-skew and churn tests prevent false `CONSUMING`; low/skewed traffic remains valid. |
