...
| Batch, lease, record-size and capacity limits are needed | Bound resource and concurrency cost | `FULL` | `ADOPTED` | Deployment limits and admission formulas cover records, parser structure, bytes, leases, mutations, snapshots and workers. | Boundary, parser, concurrent-admission and capacity-isolation tests pass. |
