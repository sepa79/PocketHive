...
| Batch, lease, record-size and capacity limits are needed | Bound resource and concurrency cost | `FULL` | `ADOPTED` | Deployment limits and admission formulas cover schema evaluation, records, parser structure, bytes, leases, mutations, snapshots and workers. | Adversarial schema, boundary, parser, concurrent-admission and capacity-isolation tests pass. |
...
| Generic versioned schema | One root schema composes exact immutable contracts and local `$defs` within a closed cost-bounded keyword profile. | Supports arbitrary domains without PocketHive business fields or unbounded evaluation. |
