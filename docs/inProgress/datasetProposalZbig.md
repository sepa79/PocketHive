...
proposed above. The canonical specification remains authoritative.

| Original pain point | Interpreted need | Status | Canonical response | Acceptance evidence or future decision trigger |
|---|---|---|---|---|
| Records must not cross SUTs | Isolate reusable data by target environment | `SATISFIED` | Scenario Binding freezes one SUT Environment and Dataset Space. | Cross-SUT discovery/admission tests fail closed. |
| Created records must survive dirty/in-use state and Redis loss | Keep one durable inventory independent of delivery adapters | `SATISFIED` | PostgreSQL owns non-expiring Managed Dataset records, state and leases; snapshots and Redis remain separate concerns. | Restart, finite-import and authority-recovery tests preserve committed records. |
| Records may have arbitrary shapes | Represent domain-neutral structured records | `PARTIAL` | Release 1 accepts one non-null JSON object per record. Array, primitive, `null` and binary roots are excluded. | Reconsider only with a concrete need plus schema, source, snapshot, WorkItem and pipeline qualification for every new root type. |
| Tests need different subsets or projections | Reuse the same canonical data for different flows | `PARTIAL` | Groups provide frozen record partitions; workflow Views provide fixed state subsets; Create Swarm selects one exact Dataset, Group and optional View. Normal scenario processing may shape its downstream payload. | Selection tests cover Groups/Views. Dataset projection expressions remain prohibited. |
| Many swarms may use one record; some flows require exclusivity | Support concurrent reuse and temporary unavailability explicitly | `SATISFIED` | `REPLAY + SHARED` permits concurrent immutable reuse. `REPLAY` or `WORKFLOW + EXCLUSIVE_LEASE` permits one active lease. | Shared fan-out and duplicate-lease concurrency tests pass. |
| Flows need durable operational metadata such as dirty/ready | Move records through declared operational states | `PARTIAL` | `WORKFLOW` uses typed Record State, fixed Views and declared transitions under an exclusive lease. Shared replay has no mutable state. | Transition tests cover the declared workflow; concurrent shared mutation is rejected. |
| Free-form tags should remain schema-valid | Prevent typos and undeclared state changes | `REJECTED` | No tag bag exists. A closed state schema plus allowed paths and transitions provides the bounded equivalent. | Unknown paths, invalid states and undeclared transitions fail atomically. |
| Redis restart must not lose authority | Keep durability outside a volatile list | `SATISFIED` | PostgreSQL is authoritative for Managed Dataset; the existing Redis Dataset remains unchanged. | Redis import/restart tests never treat the live list as Managed Dataset authority. |
| Redis should optionally distribute Managed Dataset records | Publish to an external Redis projection | `DEFERRED` | Release 1 distributes snapshots through qualified shared storage and worker memory; no outbox/projection is added. | Reconsider when a named consumer cannot use Managed Dataset or an existing explicit Redis path. |
| Definitions need stable identity, name and versions | Make contracts discoverable and immutable | `SATISFIED` | Dataset Definition, required name, opaque runtime id and exact schema-contract versions are frozen without fallback. | Authoring and compatibility tests reject changed or unsupported versions. |
| Read/allocation modes must be explicit | Prevent silent switching between shared and exclusive behaviour | `SATISFIED` | Profile and allocation are closed, frozen and capability-gated. | Unsupported or mismatched Profile/allocation fails admission. |
| Selectable/indexed fields should be declared | Bound selection semantics and cost | `REJECTED` | Dataset-level field selectors and queries are excluded; Groups and fixed workflow Views are the only subsets. | Admission accepts only an exact Dataset, Group and optional View. |
| Data classification and access must be controlled | Prevent unsafe data exposure | `PARTIAL` | Release 1 fixes `SYNTHETIC_NON_SENSITIVE`, has no per-Dataset override and authorises SUT, Dataset, Group, binding, run and operation. PocketHive does not infer sensitivity from values. | Schemas reject classification fields and surfaces expose no values; reconsider policy levels only with storage, log, backup and inspection controls. |
| Dataset must remain domain-neutral | Avoid built-in customer/card/business fields | `SATISFIED` | Record, state and Group schemas own domain fields; PocketHive defines none. | Schema tests accept arbitrary object fields within declared contracts. |
| Ordinary reads must not mutate payload | Preserve stable reusable content | `SATISFIED` | Record payload is immutable and workers load the canonical record from a verified snapshot. | Digest and replay tests prove unchanged canonical records. |
| Business data may need replacement | Change an existing record payload | `REJECTED` | `PAYLOAD_REPLACE` is excluded because it breaks immutable snapshot identity. Derivation creates new independent records. | Reconsider only through a separate versioned migration/derivation design. |
| Reservations must be separate from business metadata | Provide safe technical exclusivity | `SATISFIED` | Record Lease is authority-owned state with owner, token and expiry, never payload or Record State. | Ownership, expiry, conflict and recovery tests pass. |
| A long-running reservation may need renewal | Extend an exclusive claim beyond its admitted operation bound | `REJECTED` | Release 1 has no lease renewal. Admission requires the fixed lease duration to cover the declared operation, completion and clock bounds. | Reconsider only if a qualified workflow cannot fit a bounded lease and defines renewal fencing, failure and capacity behaviour. |
| One durable source of truth is required | Keep runtime records and mutations restart-safe | `SATISFIED` | PostgreSQL owns Managed Dataset runtime data; Scenario Manager separately owns mounted authoring contracts. | Authority and authoring-owner tests reject duplicate ownership. |
| Transition and metadata history should be retained | Reconstruct every past record-state change | `DEFERRED` | Release 1 retains current typed state/`stateRevision`, correctness idempotency/provenance and aggregate operational evidence, not an inspectable per-record transition log. | Reconsider when a proven diagnostic or audit case defines retention, authorisation, capacity and inspection. |
| All records should be discoverable | Diagnose inventory and operational health | `PARTIAL` | REST/UI/MCP expose bounded Dataset/Group counts, availability, publication and consumption status. Record browsing, value search and record-level inspection are excluded. | Reconsider when bounded status cannot answer a concrete QA/operations diagnosis and a safe paginated inspection contract is approved. |
| Worker I/O must be explicit | Keep one supported pipeline without hidden defaults | `SATISFIED` | Managed Dataset adds explicit adapters to the existing WorkInput/WorkOutput pipeline and SDK-owned typed Context. | M0 contract and capability tests reject missing/unknown adapter settings. |
| Create, mutate and release must be atomic | Avoid partial state or lease outcomes | `PARTIAL` | Record creation, declared workflow transition/release and bounded Derivation are atomic; generic tag patch and payload replacement are rejected. | Transaction, idempotency and failure-injection tests prove all-or-none outcomes. |
| PostgreSQL selectors should filter records dynamically | Choose eligible records by payload/tag values | `REJECTED` | Workers select locally from one frozen Group; workflow claims use one fixed materialised View. | No API or adapter accepts an arbitrary selector/query. |
| Throughput near 1,000 records/s should be practical | Protect traffic-generation performance at target scale | `PARTIAL` | Workers use verified local memory; PostgreSQL export and mutable authority calls stay off the measured replay path. Performance remains unproven until qualification. | Paired performance, restart fan-out and 24-hour soak gates must pass at declared scale. |
| Provider-created supply and renewal may be needed | Create reusable data without manual imports | `SATISFIED` | The MVP includes bounded `SCHEDULER + REPLAY + SHARED`; later Release 1 sources are finite CSV/Redis and bounded Derivation. | Source-specific capacity, failure and publication tests gate advertisement. |
| Dataset must not infer SUT truth | Keep business outcome ownership in the scenario | `SATISFIED` | A scenario-owned Outcome Normaliser emits one closed Outcome; only explicit workflow completion changes state. | Missing/ambiguous output changes no state; no SUT-response inference exists. |
| Authorisation scopes need definition | Restrict reads and mutations to the admitted run | `SATISFIED` | Every API authorises SUT, Dataset, Group, binding, run and operation; lease mutation requires its owner. | Cross-run/binding and stale-token tests fail closed. |
| Batch, lease, record-size and capacity limits are needed | Bound resource and concurrency cost | `SATISFIED` | Deployment limits and admission formulas cover records, bytes, leases, mutations, snapshots and workers. | Boundary, concurrent-admission and capacity-isolation tests pass. |
| REST, Worker I/O, capability and observability contracts are needed | Give delivery teams executable interfaces | `PARTIAL` | The normative design fixes owners and shapes; M0 executable schemas/types remain pending. | No implementation starts until the named M0 contracts and tests exist. |
| Inventory should be retained indefinitely | Avoid accidental data loss | `PARTIAL` | Release 1 records are non-expiring and never purged, but operation is bounded by declared storage horizon and admission limits. | A future reclamation design must define active-run, evidence and recovery safety. |
