...
The shared WorkItem codec validates every inbound and outbound envelope against that schema before
mapping it to SDK types; the schema is the sole authority for wire-field requirements and shapes.
...
Every process that reads or writes shared runtime files receives the required `POCKETHIVE_RUNTIME_FILESYSTEM_ROOT`; this is always the path visible to that process. In the production containers it is `/app/scenarios-runtime`, while an isolated harness supplies its own absolute temporary root. `POCKETHIVE_SCENARIOS_RUNTIME_ROOT` has one different meaning: it is the host bind-mount source used by the runtime adapter and is never used for local file IO. The canonical runtime-filesystem resolver validates both settings and constructs every startup, remove, journal and ownership-manifest path and bind mount.

External diagnostics, including PocketHive MCP, do not locate or parse runtime files. They consume the Orchestrator runtime-debug API; Orchestrator remains the sole owner of ownership-manifest storage, selection and serialization.

The Orchestrator writes the startup JSON under its runtime-filesystem root using a content-addressed filename. The controller container receives the required `POCKETHIVE_STARTUP_ARTIFACT_PATH` and `POCKETHIVE_STARTUP_ARTIFACT_SHA256`, reads only that file below its runtime-filesystem root, verifies SHA-256 and requires `swarmPlan.id` to equal its configured swarm id. Missing settings or files, unknown schema versions, checksum mismatches and identity mismatches fail startup. There is no AMQP, registry, relative-path or default-file fallback.
...
The machine-readable SSOT is `docs/spec/swarm-lifecycle.schema.json`. Its Java representation lives only in `common/swarm-model` under `io.pockethive.swarm.model.lifecycle`: the six axis enums, operation/result types, `ControlRequest`, `SwarmCreateRequest`, `ControlResponse`, `RuntimeMetadata`, `SwarmOperation`, `SwarmStateView`, `RemoveRequest` and `RemoveResult`. Every operation owns immutable runtime metadata when it is reserved; CREATE allocates its `runId` before any launch side effect, so a pre-registration failure can still publish and retry the canonical outcome. Services and Java clients must import those types; local lifecycle/status enums or wire DTO copies are forbidden. The shared TypeScript representation and boundary parsers live in `packages/swarm-lifecycle-contract` and are generated by `npm run contracts:generate`; UI and extension builds fail when that artifact is stale. TypeScript consumers import the package and must not normalize compatibility aliases. Control-plane envelopes remain in `common/topology-core`: `CommandResult` represents internal executor evidence and `CommandOutcome` represents the Orchestrator-owned public terminal event.

#### 5.1.1 Authority boundaries

Every row below has one authority. A module not named as the authority may call it or publish a read-only projection, but it must not duplicate its validation, transition rules, effective configuration, or success decision.

| Responsibility | Canonical authority | Allowed consumers/projections | Forbidden parallel authority |
|---|---|---|---|
| Lifecycle wire shapes and enum values | `docs/spec/swarm-lifecycle.schema.json`; shared Java representation in `common/swarm-model`; generated shared TypeScript package in `packages/swarm-lifecycle-contract` | Orchestrator REST/outcomes, Controller commands/results, UI and extension imports | Service-local lifecycle DTOs/enums, handwritten client copies, compatibility status aliases |
| Network mode selection | Required `networkMode` in the canonical create/binding/update contracts | Launch environment and runtime projections carry the selected value unchanged | Null-to-`DIRECT` coercion, compatibility constructors, parser fallbacks or invalid-value recovery |
| Operation identity and state transition | Orchestrator operation coordinator | REST `SwarmOperation`, journal history, public outcome | Controller, UI, journal adapter or listener independently completing an operation |
| Terminal operation audit evidence | Orchestrator synchronous terminal-evidence port with explicit `swarmId` and `runId` | Configured durable journal adapter | Runtime-directory files, an asynchronous best-effort buffer, or lookup of `runId` from a registry entry that cleanup is about to remove |
| Runtime and workload intent | Orchestrator lifecycle application service | `SwarmStateView`, Controller command input, UI | Status listeners, Controller observations or UI actions mutating intent directly |
| Expected worker set and worker observations | Controller lifecycle core loaded from the verified startup artifact | Controller status projection, convergence evaluator, queue/worker UI views | A wrapper core, listener or scenario runner maintaining another readiness registry |
| Workload observation and readiness | The same Controller lifecycle core that owns expected workers | Orchestrator observation cache and UI projections | Separate `ManagerRuntimeCore`/Controller state fields or independently updated readiness trackers |
| Controller lifecycle capabilities | Mandatory command/observation capabilities composed by `SwarmLifecycle`; scenario and buffer-guard projections are separate required interfaces | Spring lifecycle facade and listener | Default interface methods that silently return success, `null`, empty state or perform no action |
| Queue observation value | `common/manager-sdk` `QueueStats` | Buffer guard, Controller metrics and scenario projection | Controller-local queue-stat records or field-by-field conversion between equivalent values |
| Runtime resource observation | The configured compute adapter for worker/Controller runtime and the configured topology adapter for RabbitMQ resources; their resource types are non-overlapping | Remove coordinator combines their immutable observations | Cleanup code treating a delete invocation or intended resource list as observed absence |
| Runtime filesystem layout and effective roots | Shared runtime-filesystem contract and resolver | Startup artifact, remove handshake, journal and ownership-manifest filesystem adapters | Service-local path constants, relative defaults, repeated environment parsing or reconstructed container paths |
| Filesystem persistence implementation | One reusable runtime-filesystem infrastructure adapter | Orchestrator and Controller ports | Jackson/NIO persistence copied into services or placed in transport-free domain core |
| Public lifecycle response construction | One Orchestrator operation-response factory based on the accepted operation | Swarm, component and manager REST controllers | Controller-local `new ControlResponse(...)` builders |
| Public terminal outcome | Orchestrator operation coordinator after canonical postconditions pass | AMQP outcome, REST polling, journal | Executor result, cleanup adapter, listener or UI inferring public success |
| SUT environment contract | `docs/spec/sut-environment.schema.json`; the registry array only references it from `sut-environments.schema.json`; the sole Java representation is `common/swarm-model` `SutEnvironment`/`SutEndpoint` | Scenario Manager registry and bundle APIs, Orchestrator, Controller and tools consume the same values | Scenario Manager wire/domain DTOs, copied schema definitions, service-local validation, compatibility shapes or UI metadata embedded in the runtime contract |
...
| HTTP request template contract | `common/request-templates` `HttpTemplateDefinition` and `RequestTemplateParser`; loading in `request-template-files` | Request Builder, HTTP Sequence, Scenario Manager tooling | Same-named worker-sdk records or authoring metadata embedded in a parallel runtime contract |
...
#### 5.1.2 Projection rules

A projection is derived, read-only and replaceable. It must name its source authority and may not become a second decision point.

| Projection | Source authority | Permitted use | Must not do |
|---|---|---|---|
| Controller `status-full`/`status-delta` | Controller lifecycle core | Publish worker observations, readiness, workload observation and health | Own intent or operation state |
| Scenario runtime view | Controller lifecycle core | Give scenario evaluators a read-only workload/worker snapshot; scenario actions call a command-only enable/disable port back into that same core | Keep scenario-local workload state, readiness tracking, expected workers or transition rules |
| Orchestrator `SwarmStateView` | Orchestrator intent/operation plus latest Controller and adapter observations | REST/UI query model | Drive transitions from a collapsed display state |
| UI lifecycle badge | `SwarmStateView` | Human-readable rendering | Submit or infer hidden lifecycle transitions |
| Journal record | Accepted operation and its evidence | Audit/history | Complete, retry or repair the live operation |
| Scenario Manager SUT registry | Canonical `SutEnvironment` values | Authoring and selection | Redefine runtime fields or validation; UI-only presentation metadata belongs to a separate future projection if it is needed |

Projection code may rename or aggregate fields only at its boundary. If two consumers need the same transformation, that transformation has one shared owner; copying a mapper or response factory is not a projection.

#### 5.1.3 Canonical success postconditions

Only the Orchestrator operation coordinator changes an operation to `SUCCEEDED` and publishes the public terminal outcome. Executor results are evidence, not public success.

| Operation | Required evidence before `SUCCEEDED` |
|---|---|
| `CREATE` | Matching Controller identity and startup digest, Controller `READY`, complete fresh worker set, and initial workload observation `STOPPED` |
...
For `REMOVE`, cleanup order is part of the contract:
...
#### 5.1.4 Replacement and verification gate

Single-authority migration is replacement, not synchronization:

- the new owner and deletion of the previous owner land in the same mergeable slice;
- shadow state, dual writes, compatibility mappers and temporary fallback readers are forbidden;
- every changed responsibility has a named owner, repository-wide duplicate search and tests for invalid/failure paths;
- a comparative `code-quality-review` HiveMap scan must use coverage equivalent to the baseline and resolve the targeted fingerprint without adding `duplicate-authority`, `contract-drift` or `unsafe-failure` findings;
- local integration uses the documented ingress/API/MCP surface;
- production-like gates use the `.54` HiveForge deployment and verify lifecycle, idempotency, runtime/topology diff and cleanup evidence through PocketHive MCP;
- evidence and unresolved risks are recorded in HiveMind.

A slice that requires two active owners is not partially complete; it has not passed the architecture gate and must not merge.
...
- Core state owners reject null, blank or structurally invalid input at their public boundary. Unknown but valid identities may produce an explicit not-found result; invalid identities must not be converted into not-found, no-op or empty-success behavior.
- A configured Controller journal is mandatory. Cross-swarm entries and serialization or append failures are synchronous failures; only an explicitly injected `SwarmJournal.noop()` is allowed to disable persistence in tests or an intentionally configured no-journal composition.
...
The Orchestrator emits the public terminal remove outcome only after it has read and validated the matching result. It removes and verifies the Controller runtime, deletes the swarm runtime directory, removes the active registry entry, synchronously persists terminal audit evidence outside that directory using the captured `runId`, and only then publishes the outcome. A missing result reaches `TIMED_OUT`; it never becomes success. Controller/Orchestrator crash recovery and cross-restart reconciliation are explicitly outside this version of the contract.
...
sequenceDiagram
  ...
  MSH->>RT: Query resulting worker and Rabbit resource observations
  MSH->>FS: Create immutable success or failure result
  QN->>FS: Read and validate matching result
  alt Success
    QN->>RT: Remove Controller
    QN->>RT: Verify Controller is absent
    QN->>FS: Delete swarm runtime directory
    QN->>QN: Remove active registry entry
    QN->>QN: Persist terminal audit evidence with captured runId outside runtime directory
