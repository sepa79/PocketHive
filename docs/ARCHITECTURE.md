...
- Declares the shared hive exchange `ph.{swarmId}.hive` and **exclusively** provisions the `ph.work.{swarmId}.*` queues plus their bindings; workers must not declare or bind hive traffic queues. See §3 and the [AsyncAPI spec](spec/asyncapi.yaml) for the canonical routing definitions.
