...
PocketHive UI / PocketHive MCP / VS Code
  ...
  → command outcomes return through AMQP
...
behavior and use shared SDK contracts. A controller start/stop outcome records
dispatch; fresh aggregate worker evidence is required to confirm convergence.
...
5. `docs/reviews/` records evidence from a particular review.
   `docs/inProgress/`, `docs/todo/`, `docs/toBeReviewed/`, and `docs/archive/`
...
| `orchestrator-service` | Desired swarm state, public lifecycle/config REST, Scenario Manager resolution/preparation for create, controller creation, admission, journals, runtime debug, and governed cleanup. | [Orchestrator REST](ORCHESTRATOR-REST.md), [architecture](ARCHITECTURE.md); `SwarmController`, `ComponentController`, `ContainerLifecycleManager`, `ControllerStatusListener` | `SwarmControllerTest`, `ContainerLifecycleManagerTest`, `ComponentControllerTest`, `SwarmEventFlowIntegrationTest` |
| `swarm-controller-service` | One swarm: applies plan, declares work topology, provisions workers, sends bootstrap config to each concrete worker, broadcasts start/stop enablement, and aggregates worker state. Dependency order is computed but is not currently enforced by the swarm-wide lifecycle dispatch. Controller lifecycle state/outcome records dispatch, while fresh aggregate status is the worker-convergence evidence. | `SwarmSignalListener`, `SwarmRuntimeCore`, `SwarmLifecycleManager`, `SwarmWorkTopologyManager`, `DockerWorkloadProvisioner` | `SwarmSignalListenerTest`, `SwarmLifecycleManagerTest`, `SwarmLifecycleManagerIntegrationTest`, `SwarmRuntimeCore*Test` |
...
live-update outcome returns to the Orchestrator; worker status is consumed by
...
| Module | Owns |
| --- | --- |
| `common/swarm-model` | Canonical Java scenario/runtime model: bees, template, plan, topology, SUT, and network DTOs. |
...
and the [architecture traceability table](ARCHITECTURE.md#12-workflow-traceability).
