...
This folder contains the current reference material for the PocketHive MCP and
IDE integrations. Historical build plans and POC material live under
`docs/archive/pockethive-plugin/`; future IntelliJ work lives in
`docs/todo/pockethive-intellij-plugin.md`.
...
| 9 | `AI-ASSISTANT-SETUP.md` | Assistant client configuration |
| 10 | `BUNDLE-WIZARD.md` | Implemented novice flow and remaining UX direction |
| 11 | `VSCODE-PLUGIN.md` / `VIEWS.md` | Current IDE implementation reference |
...
The sequence below is historical context for the current product shape, not an
open checklist. Remaining work must be tracked in `docs/todo/` or carry an
...
3. Build the IDE adapters.
   - VS Code spawns or connects to the MCP server.
