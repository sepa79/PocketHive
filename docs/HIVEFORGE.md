...
Missing or invalid values fail deployment validation. The renderer passes the same allowance to
Auth Service and MCP and rejects remote HTTP without it. This is an unencrypted
exception; the [public endpoint transport contract](architecture/AUTH_SERVICE_API_SPEC.md#public-endpoint-transport-policy)
...
without a scheme or path. The two MCP/Auth Service secrets
...
Both `deploy` and `update` load these settings from
